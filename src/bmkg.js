  ...
  weather: {
    ...
    single: provinceID => {
      return new Promise(async (resolve, reject) => {
        try {
          const url = helpers.getWeatherLink(provinceID);
          const response = await fetch(url);
          const json = helpers.convertToObject(await response.text());
  ...
  earthquake: {
    ...
    latest: () => {
      return new Promise(async (resolve, reject) => {
        try {
          ...
          const json = helpers.convertToObject(await response.text());
          const unordered = helpers.formatResponse(json.Infogempa.gempa, true);
          const ordered = {};
      ...
      return new Promise(async (resolve, reject) => {
        try {
          ...
          results.forEach(earthquake => {
            let unordered = helpers.formatResponse(earthquake, false);
            let ordered = {};
            Object.keys(unordered)
              .sort()
    ...
    last20: () => {
      return new Promise(async (resolve, reject) => {
        try {
          ...
          let earthquakes = [];
          results.forEach(earthquake => {
            let unordered = helpers.formatResponse(earthquake, false, true);
            let ordered = {};
            Object.keys(unordered)
              .sort()
              .forEach(function(key) {
    ...
    lastWithPotentialTsunami: () => {
      return new Promise(async (resolve, reject) => {
        try {
          const response = await fetch(
            "http://data.bmkg.go.id/lasttsunami.xml"
          );
          const json = helpers.convertToObject(await response.text());
          const unordered = helpers.formatResponse(
